								...
								<?php
									...
									if (isset($_POST['submit'])){
										$message=NULL;
										 ...
										 if(empty($_POST['sensorName'])){
											 $message .= '<p>Empty Name';
											 $sensorName=FALSE;
										 }else $sensorName = $_POST['sensorName'];
										 
										  if(empty($_POST['rpiConnection'])){
											 $message .= '<p>Empty rpiConnection';
											 $rpiConnection=FALSE;
										 }else $rpiConnection = $_POST['rpiConnection'];
										 
										  if(empty($_POST['sensorType'])){
											 $message .= '<p>Empty sensorType';
											 $sensorType=FALSE;
										 }else $sensorType = $_POST['sensorType'];
	
										
										if(!isset($message)){
										
											$query1="insert into sensors(sensorName,rpiID,sensorTypeID) 
															values ('$sensorName','$rpiConnection','$sensorType')";
											...
											if ($result) {
												...
												echo'
													<div class="alert alert-success">
														...
														<strong>Success!</strong> Success Adding '.$sensorName.' sensor.
													</div>
													...
													echo'
														<div class="alert alert-danger">
															<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
															<strong>Error!</strong> Error '.$sensorName.' sensor.
														</div>
										...
										}/*End of main Submit conditional*//*End of main Submit conditional*/
									...
									<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                                        <div class="form-group">
                                            <input name="sensorName" class="form-control" placeholder="Edit Sensor Name">
                                        </div>
										<div class="form-group">
                                            <label>RPI to connect<label>
                                            <select name="rpiConnection" class="form-control">
                                              <?php
												require_once('mysteryDB_connect.php');
												
												$query1= "select rpiID,rpiName from rpi;"; // Run your query
												...
												$rpiName = $row['rpiName'];
												$rpiID = $row['rpiID'];
												echo "<option value=". $rpiID .">".$rpiName."</option>";
												}
											?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="sensorType">
                                               <?php
												...
												$query1= "select sensorTypeID,sensorType from sensortypes;"; // Run your query
												...
												echo "<option value=". $sensorTypeID .">".$sensorType."</option>";
												}
											?>
                                            </select>
                                        </div>
